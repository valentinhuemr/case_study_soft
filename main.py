...
if tabs == "Geräteverwaltung":
    ...
    try:
        ...
        if devices:
            st.write("Devices loaded successfully:", devices)  # Debugging

            # Dropdown-Menü mit Gerätenamen und IDs
            selected_device = st.selectbox(
                "Wähle ein Gerät aus",
                [f"{device.device_name} (ID: {device.id})" for device in devices]
            )

            # Gerät auswählen
            selected_device_obj = next(
                (device for device in devices if f"{device.device_name} (ID: {device.id})" == selected_device),
                None
            )
            ...
            if selected_device_obj:
                ...
                st.warning("Kein Gerät ausgewählt.")
        else:
            st.info("Keine Geräte vorhanden.")
    except Exception as e:
        st.error(f"Fehler beim Laden der Geräte: {e}")
